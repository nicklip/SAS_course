...
*******************************************/
libname learn 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)';
...
*Data set SCHOOL;
data learn.school;
   infile 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)\school.txt' firstobs=3;
   input @1  StudID       $5.
         @6  Name        $15.
         @21 Quiz1         3.
         @24 Quiz2         3.
...
*Data set SURVEY;
data learn.survey;
   infile 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)\survey.txt' pad;
   input ID : $3.
         Gender : $1.
         Age
         Salary
...
*Program to create temporary SAS data sets PRICES and
 NEW15DEC2005 used in chapter 10;
data prices;
...
*Data set BLOOD;
data learn.blood;
   infile 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)\blood.txt' truncover;
...
*Data set GRADES;
data learn.grades;
   infile 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)\numgrades.txt';
   input ID $ Grade @@;
run;

*Data set GYM;
data learn.gym;
   infile 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)\gym.txt' truncover;
   input Subj : $3.
         Date : mmddyy10.
         Fee;
...
;
proc print data=learn.inventory;
run;
ods html body = 'body_sample.html' 
 contents = 'contents_sample.html' 
 frame = 'frame_sample.html' 
 path = 'C:\Users\Nicholas\Documents\Documents\My SAS Files(32)' (url=none);
*Problem 1;
proc sql;
	select * from learn.inventory 
	where Price>20;
quit;
*Problem3;
...
*Problem4;
proc sql;
	select a.custnumber, a.model, 
	a.quantity, b.price,
	b.price*a.quantity as cost
	from learn.purchase a left join learn.inventory b
	on (a.model=b.model);
quit;
*Problem 6;
proc print data=learn.inventory;
run;
proc print data=learn.newproducts;
run;
proc sql;
	create table allproducts as
	select * from learn.inventory
		union all
	select * from learn.newproducts;
quit;
proc print data=allproducts;
run;
*Watnik Problem;
...
run;
proc sql;
	create table lsmeans as 
	select distinct yhat as means, a as a, b as b
	from myout;
quit;
proc print data=lsmeans;
run;
proc sql;
	select mean(means) as lsmeansA	
	from lsmeans where a=1
		union
	select mean(means) as lsmeanA2	
	from lsmeans where a=2
		union
	select mean(means) as lsmeanA3
	from lsmeans where a=3;
quit;
proc sql;
	select mean(means) as lsmeansB	
	from lsmeans where b=1
		union
	select mean(means) as lsmeanb2	
	from lsmeans where b=2;
quit;
ods html close;
